      ...
      if(mPtrViECodec->GetCodec(idx, video_codec) == 0)
      {
        ...
        if(codecConfigList[i]->mName.compare(payloadName) == 0)
        {
          ...
          if(mPtrViECodec->SetReceiveCodec(mChannel,video_codec) == -1)
          {
            ...
            CSFLogError(logTag, "%s Successfully Set the codec %s, video_codec.width = %d, video_codec.height = %d", __FUNCTION__,
                        codecConfigList[i]->mName.c_str(), video_codec.width, video_codec.height);
